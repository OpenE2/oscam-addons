...
TrunkUrl="http://www.streamboard.tv/svn/oscam-addons/oscam-picons/skyndas/098C"
TrunkFile='Skyndas_098C.zip'
#
# 3.
...
printHelp() {
  ...
  echo "$hr"
  echo 'Script with the use temporary directory for downloaded picons (CAID:098C - Sky De) from SVN.'
  echo ''
...
if [ -s "$InfoFile" ]; then
	# Print Info about PREVIOUS version of picons for CAID 098C
	InfoRevision=$(parseInfoTxt "Revision")
	InfoAuthors=$(parseInfoTxt "Authors")
	InfoDate=$(parseInfoTxt "Date")

	# Print to display about the PREVIOUS version of picons for CAID 098C
	echo ""
	echo "Current installed version of picons for CAID-098C:"
	echo "$hr"
	...
	# Remove the previous files
		# Print to display
		echo ""
		echo "Deleting old (unused) files of picons for CAID-098C"
		echo "$hr"
...
if [ -s $InfoFile ]; then
	# Print to display about the NEW installed version of picons for CAID 098C
  InfoRevision=$(parseInfoTxt "Revision:")
  InfoAuthors=$(parseInfoTxt "Authors:")
  InfoDate=$(parseInfoTxt "Date:")
